...
def re_migrate(tasks):
    ...
    for task in tasks:
        ...
        sql = sql_base.format(schema=schema, table=table)
        rank = 1
        for df in pd.read_sql_query(sql, con=engine114, chunksize=10000):
            df.to_sql(name=table, con=engine("raw_tb_comment_notag"), schema=schema, index=False, if_exists="append")
            print(f"已完成{rank}轮")
            rank += 1
