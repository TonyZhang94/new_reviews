        ...
        for i in range(n):
            for j in range(i, n):
                if i == j:
                    ...
                    try:
                        self.simi_np[i, j] = self.simi_np[j, i] = self.get_similarity(self.idmap[i],self.idmap[j])
                    except:
                        self.simi_np[i, j] = self.simi_np[j, i] = -1
    
    def get_vector(self,word):
        vetcor = np.zeros([self.word_vectors.vector_size],np.float32)
        for w in word:
            if w in self.word_vectors.vocab:
                vetcor = vetcor + self.word_vectors[w]
        vetcor = vetcor / len(word)
        return vetcor
        
    def get_similarity(self,w1,w2):
        w1_vector = self.get_vector(w1)
        w2_vector = self.get_vector(w2)
...
#                 correct_name = data_synonym_global.get(word,name)
                ...
                if word in self.text:
                    undouble_list = list(set(self.text[word]))
...
if __name__ == '__main__':
    s = Simlarity()
    s.run(pcid=100, cid=2018101516, path="base.vector", alpha=0.6)
