...
from reviews.script.db_operation import db_99_server, db_99_get, get_table_size_v2
from reviews.script.identify_words.filter_noise import FilterNoisy
...
class OpinionExtraction(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            # 提取词组
            # process.identify_target_phrases(limited_num=0,is_add = self.is_add)
            # 计算词频
            ...
            if not self.is_add:
                NewDBData[itemid] = [row + [i] for i, rows in process.NewDBData.items() for row in rows if not noisyFilter.isNoisyWord(i)]
