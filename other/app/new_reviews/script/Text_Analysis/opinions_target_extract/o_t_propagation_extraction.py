    ...
    def identify_target_phrases(self, n=1, limited_num=1,is_add=False):
        ...
        for pharse, wordslist in Select_Pharse.items():
            ...
            for words in wordslist:
                ...
                self.features_opinion[pharse] = self.features_opinion.setdefault(words[-1], [])
                for sentence_tree in self.Prase_Reviews_Data:
                    i = 0
                    w = words[0]
                    if w in sentence_tree.node_dict:
...
#         return text
    ...
    def get_train_from_sentence(self,review,target_node,opinion_node):
        ...
        def cleanText(line):
            ...
            line = re.sub(r"[~0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz\\/]*\(.{0,5}\)[~0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz\\/]*",'',line)
        ...
        while start < len(clean_text):
            if ('A' <= clean_text[start] <= 'Z'):
                ...
                while start < len(clean_text):
                    ...
                    while len(raw_text) > tmp_itr and clean_text[start:start+len(raw_text[tmp_itr][0])] != raw_text[tmp_itr][0]:
                        tmp_itr += 1
