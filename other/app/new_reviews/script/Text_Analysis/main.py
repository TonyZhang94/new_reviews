...
from new_reviews.script.db_operation import db_99_server, db_99_get
from new_reviews.process.analysis import OpinionExtraction, push_data
from new_reviews.script.classifier_product.keyword_classification import predict
from new_reviews.script.similarity.similarity_cluster import Simlarity, generate_sample_sents, SAMPLE_SEN
from new_reviews.script.identify_words.filter_noise import FilterNoisy


def ExtractTargetsAndOpinions(kwargs):
    pcid, cid = kwargs["pcid"], kwargs["cid"]
    msg_queue = queue.Queue(100)
    producer = threading.Thread(target=push_data, name='getSqlFromDB', args=(pcid, cid, msg_queue))
...
def task(**kwargs):
    ...
    if is_add:
        print("Enter STEP2")
        # 新增关键词
        settings = kwargs.get('settings', {"feature_limit": 10, "cluster_alpha": 0.85})
        # 保存机器学习打标数据到本地
        ...
        if is_cluster:
            ...
            Word2Vec_model_path = "E:/研究生/老师公司/zhuican_web/base.vector"
            s = Simlarity(ml_text)
            s.run(pcid=pcid, cid=cid, path=Word2Vec_model_path.format(cid), alpha=settings["cluster_alpha"])
        print("步骤二完成")
    else:
        print("Enter STEP4")
        # 观点词词语映射
...
def generate_data(pcid, cid, NewDBData, T_Seed, O_Seed, O_Map):
    ...
    for itemid in NewDBData:
        for row in NewDBData[itemid]:
            ...
            o = row[2]
            w = row[3]
            if w in T_Seed and o in O_Seed:
                ...
                table_list.append((pcid, cid, itemid, w, T_Seed[w], o, 1, opinion_pos, date, id))
    return table_list
...
def write_back_original_targets(pcid, cid, features, ml_text, IsNoise, predict_result, target_frequency, limit_num=2):
    ...
    db_psql(engine, sql, params=data)
...
def write_back_review_analysis(data, dst_table):
    ...
    create_dst_table(engine, dst_table)
    sql = (
        "INSERT INTO {}(pcid,cid,itemid,target,tag,opinion,frequency,grade,comment_date,comment_id)"
        " VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    ).format(dst_table)
    postgres_save_df(pd.DataFrame(data, columns=[
